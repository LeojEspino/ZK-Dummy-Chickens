...
function script.Create()
    ...
    GG.UpdateUnitAttributes(unitID)
    Spring.SpawnCEG ("spawning2_tank", x, y, z, 10, 1, 1, 1, 0)
    spawned = true
	...
	while true do
	    ...
	    if step then
		    if not aiming then
				...
				Move(larm, y_axis, 0, 20)
				Move(larm, z_axis, 0, 20)
				Turn(rarm, x_axis, math.rad(90), 6)
				Turn(rarm, y_axis, math.rad(0), 6)
				Turn(rarm, z_axis, math.rad(15), 6)
				Move(rarm, y_axis, 5, 20)
				Move(rarm, z_axis, -5, 20)
			end
		    ...
		    if not aiming then
				...
				Turn(head, y_axis, math.rad(4), 6)
				Turn(head, z_axis, math.rad(15), 6)
				Turn(larm, x_axis, math.rad(90), 6)
				Turn(larm, y_axis, math.rad(0), 6)
				Turn(larm, z_axis, math.rad(-15), 6)
				Move(larm, y_axis, 5, 20)
				Move(larm, z_axis, -5, 20)
				Turn(rarm, x_axis, math.rad(30), 6)
				Turn(rarm, y_axis, math.rad(-30), 6)
				Turn(rarm, z_axis, math.rad(0), 6)
				Move(rarm, y_axis, 0, 20)
				Move(rarm, z_axis, 0, 20)
			end
		    Turn(lleg, x_axis, math.rad(-60), 12)
		    Turn(rleg, x_axis, math.rad(60), 12)
...
function script.FireWeapon(num)
    if not swung then
		...
		Turn(head, z_axis, math.rad(-45), 24)
		Turn(rarm, x_axis, math.rad(45), 24)
		Turn(rarm, y_axis, math.rad(-75), 24)
		Sleep(200)
	...
	elseif swung then
	    ...
	    Turn(body, z_axis, math.rad(-45), 24)
		Turn(head, z_axis, math.rad(45), 24)
		Turn(larm, x_axis, math.rad(45), 24)
		Turn(larm, y_axis, math.rad(75), 24)
		Sleep(200)
		Move(body, y_axis, 0, 30)
	end
end
