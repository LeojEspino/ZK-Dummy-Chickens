...
function script.Create()
    ...
    GG.UpdateUnitAttributes(unitID)
    Spring.SpawnCEG ("spawning2_assassin", x, y, z, 10, 1, 1, 1, 0)
    spawned = true
...
function script.AimWeapon(num, heading, pitch)
    ...
    if not spawned then
	    ...
	    if not swung then
		    ...
		    Turn(larm, x_axis, math.rad(0), 12)
		    Turn(rarm, x_axis, math.rad(90), 24)
		    Turn(rarm, y_axis, math.rad(-30), 24)
...
function script.FireWeapon(num)
    if not swung then
		...
		Turn(rarm, x_axis, math.rad(30), 48)
		Turn(rarm, y_axis, math.rad(30), 48)
		Turn(rarm, z_axis, math.rad(-90), 48)
		Sleep(300)
